...
class ProductoRepository implements ProductoRepositoryInterface
{
    ...
    ): LengthAwarePaginator
    {
        ...
        if (
            isset($filters['categoria_producto_id'])
            && $filters['categoria_producto_id'] !== ''
        ) {
            $query->where(
        ...
        if (
            isset($filters['unidad_medida_id'])
            && $filters['unidad_medida_id'] !== ''
        ) {
            $query->where(
                'unidad_medida_id',
                $filters['unidad_medida_id']
        ...
        if ($bodegaId !== null) {
            ...
            $query->with([
                'inventarios' => function ($q) use ($bodegaId) {
                    $q->where(
                        'bodega_id',
                        $bodegaId
                    );
